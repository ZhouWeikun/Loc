...
def batch_rotate_images_per_sample(images, angles_deg):
    ...
    # 应用旋转
    rotated_images = F.grid_sample(images, grid, mode='bilinear', padding_mode='zeros', align_corners=False)
...
def batch_rotate_images(images, angles_deg):
    ...
    # 应用旋转
    rotated = F.grid_sample(images_expanded, grid, mode='bilinear', padding_mode='zeros', align_corners=False)
...
def rotate_with_torchvision(images, angles_deg):
    ...
    for angle in angles_deg:
        # 对所有图像应用当前角度
        rotated = TF.rotate(images, angle, interpolation=TF.InterpolationMode.BILINEAR, fill=0)
