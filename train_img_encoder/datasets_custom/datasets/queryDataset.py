...
# def getPosByAngle(img, angle, output_size=(512, 512)):
#     h, w, c = img.shape
#     y_center = h // 2
#     x_center = w//2
#     r = h // 2
...
class RotateAndCrop(object):
    def __init__(self, rate, output_size=(224, 224), rotate_range=360, warpsize=(512, 512)):
        self.rate = rate
    ...
    def __call__(self, img):
        ...
        def getPosByAngle(img, angle): #同时完成旋转+缩放；缩放比例=output_size/h
            h, w, c = img.shape
            y_center = h // 2
            x_center = w//2
            r = h // 2
            ...
            pointsListAfter = np.float32(
                [[0, 0], [0, self.output_size[0]], [self.output_size[0], self.output_size[1]], [self.output_size[1], 0]])
            M = cv2.getPerspectiveTransform(pointsOri, pointsListAfter)   # 计算从原始四个点（pointsOri，即旋转后的角点）到目标四个点（pointsListAfter，即裁剪区域）的透视变换矩阵 M
            res = cv2.warpPerspective(
                img, M, (self.output_size[0], self.output_size[1])) # M同时完成了旋转+比例缩放
            return res
...
if __name__ == '__main__':
    ...
    sat_img_path = "/home/data/zwk/dataset_xiangan/dataset_xmu_university1652_form_uavimgsize512_satimgsize204/test/gallery_satellite/000006/24.603380495694445_118.29355790069444_16x00007.tif"
    uav_img_path = "/home/data/zwk/dataset_xiangan/dataset_xmu_classify_uavimgsize512_satimgsize204/1119_0016/x_downscale8/16x00001.JPG"
    image = Image.open(uav_img_path)

    transform_uav_list1 = [ RotateAndCrop(rate=1.0,output_size=(224,224)) ]
    transform_uav_list2 = [
        transforms.Resize(224, interpolation=3),
        transforms.RandomRotation(360, interpolation=3),
        transforms.CenterCrop(224)
    ]

    trans_uav1 = transforms.Compose(transform_uav_list1)
    trans_uav2 = transforms.Compose(transform_uav_list2)
    img_raced = trans_uav2(image)

    # re = RandomErasing(probability=1.0)
    # from torchvision import transforms
    # ra = transforms.RandomAffine(180)

    # rac = RotateAndCrop(rate=1.0,output_size=(224,224))
    # img_raced = rac(image)
    plt.imshow(img_raced)
    plt.show()

    from torchvision import transforms
    rotate_crop = transforms.Compose([
        transforms.RandomRotation(180, expand=True),  # 随机旋转
        transforms.CenterCrop((512,512)),
    ])
    time_begin1 = time.time()
    img_raced3 = rotate_crop(image)
    time_end1 = time.time()
    plt.imshow(img_raced3)
    plt.show()
