...
class RandomRotationWithAngle(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        # 检查输入的维度。通常单个图像是 [C, H, W] (ndim=3)，Batch是 [B, C, H, W] (ndim=4)
        if img.ndim == 4:
            # 输入是一个Batch
            batch_size = img.shape[0]

            # 1. 为Batch中的每张图片生成一个随机角度列表
            ...
            # 处理PIL Image等其他情况或报错
            # 为了简化，我们假设输入总是3D或4D的Tensor
            # 如果也需要处理PIL Image, 可以用 isinstance(img, torch.Tensor) 来判断
...
def mk_sat_tensor_transform(
    ...
    if rand_rot:
        # 创建我们自定义的类的实例
        rotator = RandomRotationWithAngle(degrees=180)
        # 将这个实例加入到变换列表中
