...
def evaluate(
    ...
    img_features_query = predict(config, model, query_loader) #todo:
    ...
    gallery_idx = {} #mapping the sattiles to the flattened ids
    for idx, gallery_img in enumerate(gallery_list):
        gallery_idx[gallery_img] = idx

    matches_list = []
    for query_i in query_list:
        pairs_list_i = pairs_dict[query_i] #pairs_dict comes from drone2sate dict, pairs_list_i is the satellite_labels of query_i
        matches_i = []
        for pair in pairs_list_i:
            matches_i.append(gallery_idx[pair]) #matchs_i stores the label_ids of the flattened sattiles
...
from dataset_gta_eval import GTADatasetEval
from torch.utils.data import DataLoader
class GTAEvaluator(object):
    def __init__(self,uav_transform,sat_transform):
        config_dict = {
            "data_root": '/home/data/zwk/dataset_Game4Loc',
        ...
        query_dataset_test = GTADatasetEval(data_root=config.data_root,
                                            ...
                                            view="drone",
                                            transforms=uav_transform,
                                            mode='pos',
                                            query_mode='D2S',
                                            filter_in_height=config.filter_in_height,
        ...
        gallery_dataset_test = GTADatasetEval(data_root=config.data_root,
                                              ...
                                              mode='pos',
                                              query_mode='D2S',
                                              filter_in_zoom=config.filter_in_zoom,
        ...
        self.config = config
        self.query_dataset_test = query_dataset_test
        self.gallery_dataset_test = gallery_dataset_test
        self.query_dataloader_test = query_dataloader_test
        self.gallery_dataloader_test = gallery_dataloader_test
    ...
    def eval_gta(self,
        ...
        config = self.config
        query_loader = self.query_dataloader_test
        gallery_loader = self.gallery_dataloader_test

        pairs_dict = self.query_dataset_test.pairs_drone2sate_dict

        query_list = self.query_dataset_test.images_name
        gallery_list = self.gallery_dataset_test.images_name
        query_center_loc_xy_list = self.query_dataset_test.images_center_loc_xy
        gallery_center_loc_xy_list = self.gallery_dataset_test.images_center_loc_xy
        gallery_topleft_loc_xy_list = self.gallery_dataset_test.images_topleft_loc_xy
        ...
        with torch.no_grad():
            for gallery_batch in gallery_loader:
                with autocast():
                    gallery_batch = gallery_batch.to(device=self.device)
        ...
        all_scores = torch.cat(all_scores, dim=1).numpy()#all_query=9067,all_gallery_len=14640

        # with image match for finer loc
        if with_match:
            matcher = GimDKM(device=self.device)

        ap = 0.0

        gallery_idx = {}  # mapping the satetiles to the flattened ids
        for idx, gallery_img in enumerate(gallery_list):
            gallery_idx[gallery_img] = idx
        ...
        for i in tqdm(range(query_num), desc="Processing each query"):
            ...
            dis_list.append(
                get_dis(query_center_loc_xy_list[i], index, gallery_center_loc_xy_list, disk_list, match_loc))
        ...
        if top10_log:
            for query_img, top10, loc, dis_ori, dis_match in zip(query_list, top10_list, loc1_list, dis_ori_list,
                                                                 dis_match_list):
                print('Query', query_img)
                print('Top10', top10)
                print('Query loc', loc[0], loc[1])
...
if __name__ == '__main__':
    evaluator = GTAEvaluator(None,None)
