...
class UAVSatPairDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # self._maybe_debug_visualize_sample(index, dict2return, save_dir='trainers/vis_results')
        return dict2return

    def _maybe_debug_visualize_sample(self, index, sample_dict, max_samples=1, save_dir='trainers/vis_results'):
...
if __name__ == '__main__':
    ...
    sat_dataset = SatDataset(
        p_satinfo_json='/path/to/visloc/04/satellite_info.json',
        p_uav_geocsv='/path/to/visloc/04/uav_geo.csv',
        imgsize2net=224,
    )

    uav_dataset = UAVDataset(
        p_uavinfo_json='/path/to/visloc/04/uav_metadata.json',
        p_uav_geocsv='/path/to/visloc/04/uav_geo.csv',
        sat_dataset=sat_dataset,
        stage='train'
    )
