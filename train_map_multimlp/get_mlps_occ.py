...
from loc_utils import compute_l2_dist
...
class MultiMLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor, nrcs=None) -> torch.Tensor:
        """
        MultiMLP 的前向传播。
        ...
        Returns:
            torch.Tensor: 经过 Softmax 后的输出概率，形状为 (batch_size, num_mlps)。
        """
        # compute the mlp_id of the region that contain nrcs
        nrcs2centers = compute_l2_dist(nrcs, self.nrcs_grid_center)
        mlp_ids = torch.argmin(nrcs2centers, dim=1)
        sorted_region_ids, sorted_indices = torch.sort(mlp_ids)
        unique_ids, counts = torch.unique_consecutive(sorted_region_ids, return_counts=True)

        # 使用排序后的索引来重排mlp_inputs, 这会将属于同一个MLP的数据排在一起
        sorted_mlp_inputs = x[sorted_indices]

        # compute the local nrcs
        # gtnrcs_gid_center = self.nrcs_grid_center[mlp_ids]
        # gtnrcs_in_grid = nrcs - gtnrcs_gid_center
        # gtnrcs_in_grid = gtnrcs_in_grid / self.gird_cell_hw[None, :].to(x.device)
        # gtnrcs_in_grid_pe = self.pos_encoder(gtnrcs_in_grid)
        # gtnrcs_in_grid = gtnrcs_in_grid[sorted_indices]

        outputs = []
        current_pos = 0
        # 这个循环的次数等于“活跃”的MLP数量，远小于总点数N
        for region_id, count in zip(unique_ids, counts):
            # 获取属于当前region_id的数据切片
            input_slice = sorted_mlp_inputs[current_pos: current_pos + count]
            mlp = self.mlps[region_id]
            output_slice = mlp(input_slice)
