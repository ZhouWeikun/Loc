...
class circorr2(nn.Module):
    ...
    def circular_corr(self, x1, x2, scale=1):
        ...
        #debug,vis the corr
        # from models.taskflow import vis_multi_rc,vis_single
        # p2save = '/home/data/zwk/pyproj_DUAV_salad/corr_2dshift_mean_uav_18x04190.jpg'
        # labels = [f'rot_{(i+1)*10}deg' if i>4 else f'rot_{i*10}deg' for i in range(35)]
        # vis_multi_rc(corr.squeeze().detach().cpu().numpy(), labels, p2save,fighw=(3,1.5))
        ...
        #debug vis the corr
        # p2save = '/home/data/zwk/pyproj_DUAV_salad/corr_2dshift_mean_uav18x04190.jpg'
        # title = f'feat=18x04190\nq=rot50deg\nr=rot10diffdeg'
        # vis_single(corr.T.squeeze().detach().cpu().numpy(), p2save,title,fighw=(4,5))
        ...
        corr_cent = torch.fft.fftshift(corr, dim=-1)
        shift = corr_cent.shape[-1] // 2 - torch.argmax(corr_cent, dim=-1)
        return corr_cent, score, shift
