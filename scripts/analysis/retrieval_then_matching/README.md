...
```bash
conda run -n gim python /home/data/zwk/pyproj_neuloc_v0/scripts/analysis/retrieval_then_matching/retrieval_then_gim_refine.py
```

Recompute recall metrics from saved matching/refine details:

```bash
conda run -n gim python /home/data/zwk/pyproj_neuloc_v0/scripts/analysis/retrieval_then_matching/export_matching_refine_recall_from_details.py
```
...
- Use the nested `scripts/analysis/retrieval_then_matching/*.py` paths for new
  commands.
