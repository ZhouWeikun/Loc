...
class SatDataset(object):
    def __init__(self,
        ...
        # for defining the scale to sample,needs csv_info
        ...
        self.coords_4d_bounds = {
            ...
            'rot': (-np.pi, np.pi),
            'scale': (self.satimgsize_scale_to_refm_boundary[0], self.satimgsize_scale_to_refm_boundary[1]),
        }
    ...
    def mk_sacle_levels(self, n_level=3, scale_mode="linear"):
        ...
        s_min = float(self.satimgsize_scale_to_refm_boundary[0])
        s_max = float(self.satimgsize_scale_to_refm_boundary[1])
    ...
    def crop_satimg_by_4d_coords(self, coords_4d, apply_rotation=True):
        ...
        if all_scale_equal:
            ...
            # Here we use the first satmap only (consistent with original random.choice behavior for tensor mode)
            satmap_tensor = self.satmaps_tensor[0]
            if satmap_tensor.device.type != 'cuda' and torch.cuda.is_available():
                satmap_tensor = satmap_tensor.cuda()
...
class UAVDataset(object):
    def __init__(self,
                 ...
                 stage='train',
                 use_augmentation=True,  # 新增：控制是否使用数据增强
