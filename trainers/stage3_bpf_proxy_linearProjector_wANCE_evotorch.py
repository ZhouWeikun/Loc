...
class MetricNetTrainer(GridHashFitTrainer):
    ...
    def __init__(self, opt=None):
        """初始化Stage 3 Trainer"""
        should_print_final_config = (opt is None) or bool(getattr(opt, 'inherit_stage2_yaml', ''))
        if opt is None:
    ...
    def _apply_inherit_stage2_yaml(opt):
        ...
        inherit_scopes = MetricNetTrainer._normalize_inherit_stage2_scope(
    ...
    def _test_3d_fine_accuracy_seed_mode_CMA_ES(
        ...
        返回:
            dict:
                {
                    "probs_fm_dist": [B, K],   # 最终 mode / CMA 分数
                    "coords_pred":   [B, K, 4],# 最终预测坐标
                    "coords_gt":     [B, 4],   # GT 坐标
                    "coords_init":   [B, K0, 4],# Stage 1.5 初始 mode
                    "scores_init":   [B, K0],  # 初始 mode 分数
        ...
        pipeline_cfg = SeedModeSearchConfig(
            ...
            metadata={"verbose_stage_timing": bool(debug_stage_timing)},
        )
        ...
        topk_seed = min(int(l0_topN), n_anchor_3d)
        mode_init_all = []
        mode_final_all = []
        query_results = []
        ...
        coords_init_all, scores_init_all = self._pack_mode_states_for_eval(mode_init_all)
        coords_pred_all, probs_pred_all = self._pack_mode_states_for_eval(mode_final_all)
    ...
    def test(self, use_train_uav=False):
        ...
        if use_seed_mode_pipeline:
            grid_dict = self.sat_dataset.estimate_grid_shape_from_overlap(self.sat_dataset.satimgsize2crop_mean,overlap=0.5)
            n_bins_4d = [int(grid_dict['grid_rows']), int(grid_dict['grid_cols']), int(36), 4] #visloc03,overlap=0.5
            n_bins_max = max(n_bins_4d)
            dict_res = self._test_3d_fine_accuracy_seed_mode_CMA_ES(
                ...
                stage1_prob_mode='ingp',
                stage1_samples_per_round=(32),
                stage1_radius_scale_per_round=(1.05),
                stage1_survival_ratio_per_round=(0.125),#序列输入，每轮迭代后剩下的候选mode
                stage1_elite_ratio=0.1, #取ceil(stage1_samples_per_round * stage1_elite_ratio) 个 elite 样本来生成 center_raw
                stage1_enable_scale_sampling=False,

                stage3_enable=True,
                cma_variant='Sep-CMA',
                cma_prob_mode='ingp',
                cma_max_input_mode=128,
                cma_init_sigma_manual=0.75/n_bins_max,
                cma_popsize=64,
                cma_iters=12,
            ...
            stage3_analysis_export_root = getattr(self.opt, "stage3_analysis_export_root", None)
...
if __name__ == "__main__":
    ...
    remaining_argv.extend( ['--p_yaml', '/home/data/zwk/pyproj_neuloc_v0/trainer_depends/configs/stage3_visloc.yaml'])  # for testing
