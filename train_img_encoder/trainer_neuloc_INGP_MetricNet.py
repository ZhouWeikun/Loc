...
class Trainer(object):
    ...
    def train(self):
        ...
        # load the ckpt for continuing train if necessary
        from tool.util_ckpt_handler import load_param
        ...
        # self.feat_gallery_index_l2 = faiss.IndexFlatL2(self.feat_q_dim)
        # self.feat_gallery_index_l2.add(self.feat_gallery_dict['feat_gallery_flatten'].detach().cpu().numpy())
        ...
        for epoch in range(begin_epoch,num_epochs):
            ...
            for it,data in tqdm.tqdm(enumerate(self.sat_dataloader)):
                satimgs, nrcs_sat, rots_rad, ratios_satimgsize_cover = data
                coords_gt = torch.concatenate([nrcs_sat, rots_rad, ratios_satimgsize_cover], 1).to(self.device)

                if len(satimgs.shape)==5:
                    coords_gt = torch.concatenate([coords_gt.unsqueeze(1), coords_gt.unsqueeze(1)], dim=1)
                    # satimg = satimg.flatten(start_dim=0,end_dim=1)
                    coords_gt_flatten =  torch.concatenate([coords_gt.unsqueeze(1), coords_gt.unsqueeze(1)], dim=1).flatten(start_dim=0,end_dim=1)
                    satimgs = satimgs.flatten(start_dim=0,end_dim=1)
                    satimg_q_ids = [coords_gt.shape[1] * i for i in range(coords_gt.shape[0])]
                    satimg_pos_ids = np.array(satimg_q_ids)+1
                else:
                    coords_gt_flatten = coords_gt
                    satimg_q_ids = None

                # get the feats_img from the vis_encoder
                feats_img = self.img_encoder(satimgs.to(self.device))
                if self.agg_name == 'g2m':
                    feats_img = self.img_aggregator(feats_img[:,1:,:].permute(0,2,1).reshape(satimgs.shape[0],self.feat_q_dim,14,14)) #for patchsize=16,imgszie=224
                elif self.agg_name == 'salad':
                    feats_img = self.img_aggregator(feats_img)

                # get the feats_grid corresponding to the coords_gt from INGP
                feats_grid = self._feats_fm_grid(coords_gt_flatten)
                coords_encoded = encode_4d_coords(coords_gt_flatten,
                ...
                # hard mining by faiss
                    # find the index of hard samples from cached gallery
                feats_q = feats_img[satimg_q_ids] if satimg_q_ids is not None else feats_img
                feat_dist, feat_index = self.feat_gallery_index.search(feats_q.detach().cpu().numpy(), k=20)
                # feat_dist_l2, indices_l2 = self.feat_gallery_index_l2.search(feats_q.detach().cpu().numpy(), k=20)
                coords_hard = self.feat_gallery_dict['coords_gallery_flatten'][feat_index.flatten(),:].reshape(*feat_index.shape,4)
                coords_hard_flatten = coords_hard.flatten(start_dim=0,end_dim=1).to(coords_gt_flatten.device)
                coords_hard_encoded_flatten = self.feat_gallery_dict['coords_gallery_encoded_flatten'][feat_index.flatten(),:].to(coords_gt_flatten.device)
                    # get the hard from INGP
                feat_ref_hard_flatten = self._feats_fm_grid(coords_hard_encoded_flatten)
                feat_ref_hard_flatten = self.grid_mlp(inputs=feat_ref_hard_flatten, condition_features= coords_hard_encoded_flatten)
                feat_ref_hard_flatten = torch.nn.functional.normalize(feat_ref_hard_flatten,dim=-1)
                ...
                coords_encoded_expanded = coords_encoded.unsqueeze(0).expand(B, B, -1).to(ref_expanded.device)  # [B, B, coord_dim]
                ...
                rc_dist_mat = coords_gt_flatten[:, :2].unsqueeze(1) - coords_hard_flatten[:, :2].unsqueeze(0)
                rc_dist_mat = torch.norm(rc_dist_mat, dim=-1)
                rc_dist_mat_np = rc_dist_mat.detach().cpu().numpy()
                r_dist_mat = coords_gt_flatten[:, 2].unsqueeze(1) - coords_hard_flatten[:, 2].unsqueeze(0)
                r_dist_mat = torch.abs(
                    torch.atan2(torch.sin(r_dist_mat), torch.cos(r_dist_mat))).squeeze()  # atan2 函数的输出范围是 [-π, π]
                s_dist_mat = coords_gt_flatten[:, 3].unsqueeze(1) / coords_hard_flatten[:, 3].unsqueeze(0)
                ...
                # compute dist_mat form coords
                rc_dist_mat = coords_gt_flatten[:,:2].unsqueeze(1) - coords_gt_flatten[:,:2].unsqueeze(0)
                rc_dist_mat = torch.norm(rc_dist_mat,dim=-1)
                r_dist_mat = coords_gt_flatten[:,2].unsqueeze(1) - coords_gt_flatten[:,2].unsqueeze(0)
                r_dist_mat = torch.abs(torch.atan2(torch.sin(r_dist_mat), torch.cos(r_dist_mat))).squeeze() #atan2 函数的输出范围是 [-π, π]
                s_dist_mat = coords_gt_flatten[:,3].unsqueeze(1) / coords_gt_flatten[:,3].unsqueeze(0)
                s_dist_mat = torch.abs(torch.log(s_dist_mat)).squeeze() #比例关系在对数空间中会变为加减关系
                udf_dist_mat = self.udf_compter.compute_udf_fm_diff(rc_dist_mat,r_dist_mat,s_dist_mat)
                positive_mat = udf_dist_mat < self.sat_dataset.halfimg_radius_nrc
                # udf_dist_mat_np = udf_dist_mat.detach().cpu().numpy()
                ...
                #     num_uniform_samples=n_uniform_samples,
                # )
                # coords_mixed = torch.concatenate([coords_gt.unsqueeze(1),coords_sampled],dim=1)
    ...
    #             optimizer.zero_grad()
    #             feat_ref = self._feats_fm_grid(coords2opt)
    ...
    #         # 保存结果
    #         with torch.no_grad():
    #             feat_ref_final = self._feats_fm_grid(coords2opt)
    #             feat_ref_final = self.grid_mlp(inputs=feat_ref_final, condition_features=encoded_coords_opted)
    ...
    def _opt_coords_topN(self, coords_topN, feat_q, n_step=500):
        ...
        for id in range(coords_topN.shape[1]):
            ...
            optimizer = torch.optim.Adam([
                {'params': [xy_param], 'lr': 1e-4},  # nrc ∈ [0,1], 基准学习率
                {'params': [rot_param], 'lr': 1e-3},  # rotation ∈ [-π,π], 约3倍
                {'params': [scale_param], 'lr': 5e-4}  # scale > 0, 可能需要更小
            ...
            for i in range(n_step):
                ...
                # 前向传播
                feat_ref = self._feats_fm_grid(coords2opt)
                encoded_coords_opted = encode_4d_coords(
                    coords2opt,
                    rc_encoder=self.rc_pos_encoder,
                    rot_endcoder=self.rot_pos_encoder,
                    scale_encoder=self.scale_pos_encoder
                ...
                # 可选：添加坐标约束
                # with torch.no_grad():
                #     xy_param.clamp_(0.0, 1.0)  # nrc范围约束
                #     rot_param.clamp_(-math.pi, math.pi)  # 旋转范围约束
                #     scale_param.clamp_(scale_min, scale_max)  # 根据你的数据集设置
            ...
            with torch.no_grad():
                ...
                feat_ref_final = self._feats_fm_grid(coords_final)
                encoded_coords_final = encode_4d_coords(
                    coords_final,
                    rc_encoder=self.rc_pos_encoder,
                    rot_endcoder=self.rot_pos_encoder,
                    scale_encoder=self.scale_pos_encoder
    ...
    def test_xy_rot_fm_INGP(self):
        ...
        if os.path.exists(p2gallery_mat):
            ...
            feat_gallery_flatten = torch.tensor(gallery_mat['feat_gallery'], dtype=torch.float32).flatten(start_dim=0,
                                                                                                          end_dim=2)
            ...
            nrcs_gallery = self.sat_dataset.crop_sat_unifrom(size2clip=self.sat_dataset.satimgsize2crop_mean,
                                                             overlap=overlap, only_nrcs=True)
            nrcs_gallery_flatten = torch.tensor(nrcs_gallery, dtype=torch.float32).flatten(start_dim=0, end_dim=1)
            delta_rot_rangle = 10
            ...
            gallery_shape = torch.Size([nrcs_gallery.shape[0], nrcs_gallery.shape[1], rots_ref.shape[0]])
            result = {'feat_gallery': feat_gallery.detach().cpu().numpy(),
                      'nrcs_gallery': nrcs_gallery,
                      'rots_ref': rots_ref.detach().cpu().numpy(),
                      'gallery_shape': np.array(gallery_shape)
                      }
            ...
            scales_q = torch.rand(n_query)*(self.sat_dataset.satimgsize_scale_to_200m_boundary[1]-self.sat_dataset.satimgsize_scale_to_200m_boundary[0])+\
                      self.sat_dataset.satimgsize_scale_to_200m_boundary[0]
            coords_q = torch.concatenate([nrcs_q, rots_q.unsqueeze(1), (torch.ones(nrcs_q.shape[0]) * scales_q).unsqueeze(1)], dim=-1)
            satimgs_q = self.sat_dataset.crop_satimgs_by_4d_coords(coords_q)
            with torch.no_grad():
                feats_q = self._get_feats_fm_imgs(satimgs_q, feat_fm_agg=True)

            # computing pred in low_res
            from eval_recall_fm_salad import compute_recall_fm_gallery_by_eucdist
            recall_dict = compute_recall_fm_gallery_by_eucdist(feats_q,feat_gallery_flatten,nrcs_q,coords_gallery_flatten[:,:2],self.sat_dataset.halfimg_radius_nrc,k_values=[1,5,10,20,50])
            pred_id = torch.tensor(recall_dict['indices_fm_feat'])

            # 迭代优化低分辨率检索结果byINGP
            N = 5
            coords_topN = coords_gallery_flatten[pred_id[:, :N]]
            coords_topN_opted = self._opt_coords_topN(coords_topN.to(self.device), feats_q.to(self.device))
            coords_best = coords_topN_opted[:, 0, :]
            # 打印nrc_recall@1
            dist_nrc_top1 = torch.norm(coords_topN[:, 0, :2] - coords_q[:, :2], dim=-1, p=2)
            nrc_loc_success_top1 = dist_nrc_top1 < self.sat_dataset.halfimg_radius_nrc
            dist_nrc_topN_opted = torch.norm(coords_best[:, :2].to(coords_q.device) - coords_q[:, :2], dim=-1, p=2)
            nrc_loc_success_opt1_opted = dist_nrc_topN_opted < self.sat_dataset.halfimg_radius_nrc
            print(f'nrc_recall@1:{nrc_loc_success_top1.sum() / coords_q.shape[0]:.5f}; nrc_recall@1_opted:{nrc_loc_success_opt1_opted.sum() / coords_q.shape[0]:.5f}')
            # 打印INGP迭代优化后的结果
            err_nrc_top1 = torch.norm(coords_topN[:, 0, :2].detach().cpu() - coords_q[:, :2], dim=-1)
            err_met_top1_mean = self.sat_dataset.halfimg_radius_meter * err_nrc_top1.mean() / self.sat_dataset.halfimg_radius_nrc
            err_nrc_top1_opted = torch.norm(coords_best[:, :2].detach().cpu() - coords_q[:, :2], dim=-1)
            ...
            print( f'err_meter_top1:{err_met_top1_mean.item():.5f}; err_meter_top1_opted:{err_met_top1_opted_mean.item():.5f}')
            rot_diff_top1 = coords_topN[:, 0, 2].detach().cpu() - coords_q[:, 2]
            ...
            rot_diff_top1_opted = coords_best[:, 2].detach().cpu() - coords_q[:, 2]
            ...
            print(f'err_rot_top1:{rot_err_top1_mean.item():.5f}; err_rot_top1_opted:{rot_err_top1_opted_mean.item():.5f}')
            norm_factor_scale = math.log(self.sat_dataset.satimgsize_scale_to_200m_boundary[1] /self.sat_dataset.satimgsize_scale_to_200m_boundary[0])
            scale_err_top1 = torch.abs(torch.log(coords_topN[:, 0, 3].detach().cpu() / coords_q[:, 3]))
            scale_err_top1 = scale_err_top1/norm_factor_scale
            scale_err_top1_opted = torch.abs(torch.log(coords_topN_opted[:, 0, 3].detach().cpu() / coords_q[:, 3]))
            ...
            get_pos_id = False
            if get_pos_id:
                # get the index of the sample nearst to query from gallery
                gt2gallery_rc_dist_mat = torch.norm(nrcs_q.unsqueeze(1) - nrcs_gallery_flatten.unsqueeze(0), dim=-1, p=2)
                pos_ids_rc_flatten = torch.argmin(gt2gallery_rc_dist_mat, dim=-1)
                pos_rows = pos_ids_rc_flatten // nrcs_gallery.shape[1]
                pos_cols = pos_ids_rc_flatten % nrcs_gallery.shape[1]
                pos_ids_rc = torch.stack([pos_rows, pos_cols], dim=0).T
                # pos_ids_rc_np = gt_ids_rc.detach().cpu().numpy()
                if rots_ref.max() < 4:
                ...
                pos_ids_rot = torch.argmin(angular_dist, dim=-1)  # Shape: (N_q)
                # pos_ids_rot_np = gt_ids_rot.detach().cpu().numpy()

                from eval_recall_fm_salad import compute_recall_by_label
                rot_recall_results = compute_recall_by_label(
                    q_labels=pos_ids_rot[nrc_loc_success_top1].unsqueeze(-1),
                    pred_labels_per_query=pred_ids_unraled[2][nrc_loc_success_top1, :10],
                    k_values=[1, 3, 5, 10],
                    title="Conditional Rotation Recall (based on radius search)"
                )
                print("\n条件旋转召回率结果:", rot_recall_results)

                gtrot2eval = rots_q[nrc_loc_success_top1]
                predrot2eval = rots_ref[pred_ids_unraled[2][nrc_loc_success_top1, 0]]
            ...
            if vis:
                ...
                map2vis = pred_agg[id_seled].reshape(gallery_shape)[:, :, pos_ids_rot[0]]
    ...
    def test_xy_rot_fm_INGP_wUAV(self):
        ...
        from dataset_wingtra_4d import UAVDataset
        uav_dataset = UAVDataset(p_uavinfo_json = self.opt.p_uavinfo_json,
                                 geo_res_m=self.sat_dataset.geo_res_m,
                                 trans_georc2nrc_func=self.sat_dataset.transfrom_georc_to_nrc,
                                 stage='test')
        uav_dataloader = torch.utils.data.DataLoader(uav_dataset, batch_size = 128,
                                                          num_workers = self.opt.num_worker,
                                                          pin_memory=True, shuffle=False, drop_last=False,
                                                          persistent_workers=True)

        for it, data in tqdm.tqdm(enumerate(uav_dataloader)):
            imgs, nrcs_sat, rots_rad, ratios_cover = data
            coords_gt = torch.concatenate([nrcs_sat, rots_rad, ratios_cover], 1).to(self.device)
            with torch.no_grad():
                feats_q = self._get_feats_fm_imgs(imgs, feat_fm_agg=True)
            break
        # from matplotlib import pyplot as plt
        # img2vis = uav_dataset.denormalize_img(imgs[0])
        # plt.imshow(img2vis)
        # plt.show()
        feats_pos = self._feats_fm_grid(coords_gt)
        ...
        topN=50
        coords_gallery_topN = coords_gallery_flatten[indices_l2[:,:topN]]
    ...
    def save_gallery_feat(self, overlap=0.5):
        self._test_ready()
        ...
        suffix = f"_overlap{overlap}_radius{self.sat_dataset.halfimg_radius_meter:.0f}m.mat"
        scipy.io.savemat(f"{os.path.dirname(self.opt.load2test)}/{os.path.basename(self.opt.load2test).replace('.pth', suffix)}", result)
