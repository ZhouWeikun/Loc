...
class RandomScaleWithParams(torch.nn.Module):
    ...
    def forward(self, img):
        ...
        if scale > 1.0:
            ...
            img_scaled = F.pad(img_resized, [pad_left, pad_top, pad_right, pad_bottom], fill=0)
...
def mk_pil_transform_with_params(
    ...
    if rand_erase:
        transform_list.append(
            transforms.RandomErasing(p=0.1, scale=(0.05, 0.2), ratio=(0.3, 3.3), value=1)
        )
...
def apply_augment_to_coords(coords_4d, rotation_deg, scale_factor):
    ...
    # 需要根据你的坐标系定义来调整符号
    rot_augment_rad = torch.deg2rad(torch.tensor(rotation_deg))
