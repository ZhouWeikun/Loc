...
class MetricNetTrainer(GridHashFitTrainer):
    ...
    def _compute_bidirectional_ranking_loss(self, dist_pred, dist_gt,
                                            use_dist_weights=True, weights=None, sigma=0.5, margin=0.05):
        """
        ...
        if use_dist_weights:
            ...
            if weights is None:
                ...
                weights[:, 0] = 10.0  # (可选) 给 GT 点 (index 0) 额外的统治力
        ...
        loss = weighted_loss.sum() / loss_norm * 10
    ...
    def train(self):
        ...
        for epoch in range(begin_epoch, num_epochs):
            ...
            for it, batch_uav in tqdm.tqdm(enumerate(self.uav_dataloader_train)):
                ...
                # B. Eikonal Loss (复用计算图和权重)


                # version1:
                # w_eikonal = torch.clamp(1.0 - w_base, 0.0, 1.0)
                # loss_eikonal = self._compute_weighted_eikonal_loss(
                #     dist_pred.view(-1),  # 展平 [2B, N] -> [2B*N]
                #     coords_input=coords_eval_flat,  # 使用已启用梯度的 flat 版本
                #     eikonal_weights=w_eikonal.view(-1)  # 展平 [2B, N] -> [2B*N]
                # )
                # version2:
                loss_eikonal = self._forward_and_compute_eikonal_loss(
                    coords_source=coords_eval ,
                    coords_gt=coords_gt,  # <--- 记得传这个
                    feats_vis=feats_vis,
                    BatchSize=BatchSize,
                    n_sample=1024,
                    use_weight=True  # <--- 开启内部计算权重
                )
                loss_eikonal_weighted = loss_eikonal * 0.05

                # C.使用 L1 Loss 对所有点进行约束
                # loss_scale_weighted = 0.0
                # loss_scale_lambda = 1 - scale_weight_scheduler.get_weight(current_step=epoch)
                # if loss_scale_lambda > 1e-6:
                loss_scale = torch.nn.functional.l1_loss(dist_pred, dist_gt) * w_rank
                loss_scale = loss_scale.sum() / (w_rank.sum()+ 1e-8)
                loss_scale_weighted = loss_scale * 0.01

                # D. 组合 Loss
                loss = loss_rank + loss_eikonal_weighted + loss_scale_weighted

                # 保存所有loss用于日志（避免在backward后访问tensor）
                loss_val = loss.item()
                loss_rank_val = loss_rank.item()
                loss_scale_val = loss_scale.item()
                loss_scale_weighted_val = loss_scale_weighted.item()
                loss_eikonal_val = loss_eikonal.item()
                loss_eikonal_weighted_val = loss_eikonal_weighted.item()
                # 反向传播
                ...
                if it % 10 == 0:
                    if self.writer is not None:
                        ...
                        self.writer.add_scalar('loss_scale', loss_scale_val, step)
                        self.writer.add_scalar('loss_scale_weighted', loss_scale_weighted_val, step)
                        self.writer.add_scalar('loss_eikonal', loss_eikonal_val, step)
                        self.writer.add_scalar('loss_eikonal_weighted', loss_eikonal_weighted_val, step)

                    # 日志输出
                    log_msg = f'Iter {it}: Loss={loss_val:.6f} (Rank={loss_rank_val:.6f}, Scale={loss_scale_val:.6f}, ScaleW={loss_scale_weighted:.6f}, Eikonal={loss_eikonal_val:.6f}, EikonalW={loss_eikonal_weighted_val:.6f})'
