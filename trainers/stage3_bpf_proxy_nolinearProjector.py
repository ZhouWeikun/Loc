...
class MetricNetTrainer(GridHashFitTrainer):
    ...
    def __init__(self, opt=None):
        ...
        # 绝对长度定义
        self.gs_sigma_nrc = 0.65/40. #n_bins=40,即单位网格长=1/40,sigma_bin=0.5
    ...
    def _init_projector(self):
        ...
        from models.projector_coord_modulation import GeoModulatedProjector
        self.projector = GeoModulatedProjector(
            feat_dim=1024,
            coord_dim=5,  # 保持宽通道
            hidden_dim=512,  # 内部升维到 2048
            num_res_blocks=2,  # 深度
            dropout=0.1
        ).to(self.device)
    ...
    def _test_3d_classification_accuracy(
        ...
        # 预计算所有候选点的网络坐标（供 projector 使用）
        coords_flat = coords_candidates.view(-1, 4)  # [N_total, 4]
        coords_linear_flat = self.coord_normer.raw_to_linear(coords_flat)
        coords_net_flat = self.coord_normer.linear_to_net(coords_linear_flat).to(self.device)  # [N_total, 5]
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                # 计算能量 (projector 接收特征 + 坐标)
                coord_chunk = 1024
                energy_chunks = []
                for start_idx in range(0, coords_net_flat.shape[0], coord_chunk):
                    end_idx = min(start_idx + coord_chunk, coords_net_flat.shape[0])
                    coords_chunk = coords_net_flat[start_idx:end_idx]
                    coords_chunk = coords_chunk.unsqueeze(0).expand(batch_size, -1, -1)
                    coords_chunk = coords_chunk.reshape(-1, coords_net_flat.shape[-1])

                    feats_vis_chunk = feats_vis.unsqueeze(1).expand(-1, end_idx - start_idx, -1)
                    feats_vis_chunk = feats_vis_chunk.reshape(-1, feats_vis.shape[-1])

                    dist_chunk = self.projector(feats_vis_chunk, coords_chunk)
                    dist_chunk = self.softplus(dist_chunk).view(batch_size, end_idx - start_idx)
                    energy_chunks.append(dist_chunk)

                energys = torch.cat(energy_chunks, dim=1)  # [B, N_total]

                # Reshape成 [B, NR, NC, Rot, Scale]
                energys_reshaped = energys.reshape(batch_size, *n_coarse)

                logits_flat = energys_reshaped.view(batch_size, -1)  # [B, NR*NC*Rot]
                pred_indices = logits_flat.argmin(dim=-1)  # [B]
    ...
    def _get_feats_fm_INGP(self, coords, coord_mode='raw'):
        ...
        # =========================================================
        # 1. 统一转换层：同时得到 Linear-4D 与 Net-5D
        #    HashGrid 用 Linear (nr, nc, theta_lin)，MLP 用 Net (nr, nc, cos, sin, s)
        # =========================================================
        if coord_mode == 'raw':
            coords_linear = self.coord_normer.raw_to_linear(coords)
            coords_net = self.coord_normer.linear_to_net(coords_linear)
        elif coord_mode == 'linear':
            coords_linear = coords
            coords_net = self.coord_normer.linear_to_net(coords_linear)
        elif coord_mode == 'net_5d':
            coords_net = coords
            theta_lin = torch.atan2(coords_net[..., 3:4], coords_net[..., 2:3]) / torch.pi
            coords_linear = torch.cat([coords_net[..., 0:2], theta_lin, coords_net[..., 4:5]], dim=-1)
        elif coord_mode == 'net_6d':
            coords_net = coords[..., :5]
            theta_lin = coords[..., 5:6]
            coords_linear = torch.cat([coords_net[..., 0:2], theta_lin, coords_net[..., 4:5]], dim=-1)
        else:
            raise ValueError(f"Unknown coord_mode: {coord_mode}")

        # =========================================================
        # 2. 构造子模块输入 (Explicit Extraction)
        # =========================================================
        # HashGrid: 线性空间前三维 [nr_n, nc_n, theta_lin]
        grid_input = coords_linear[..., :3]

        # MLP/PosEnc: 网络空间前5维
        mlp_input = coords_net[..., :5]
    ...
    def _test_3d_fine_accuracy_debug(
        ...
        with torch.no_grad():
            ...
            if topN_refine and feats_vis_for_refine.shape[0] > 0:
                ...
                # 转换为6D坐标
                coords_topN_6d = self.coord_normer.raw_to_net(
                ...
                if enable_resample_around_topK:
                    ...
                    # 转换为6D坐标
                    coords_resampled_6d = self.coord_normer.raw_to_net(
    ...
    def _visualize_enerey_of_coords(
            self,
            coords_samples,
            energys,
            coord_gt,
        ...
        Args:
            ...
            energys: [M] 采样点对应的值 (可以是能量、距离、权重等)
            coord_gt: [4] GT坐标
            coord_pred: [4] (可选) 预测坐标。若为None则不绘制预测点。
        """
        import matplotlib.pyplot as plt
        import os
        import time
        ...
        plt.colorbar(scatter, ax=ax1, label='Value', fraction=0.046)
        ...
        # ===== 子图2: 值 vs 距离 (Value Distribution) =====
        ax2 = axes[1]
        ...
        ax2.plot(distances[min_dist_idx], energys_np[min_dist_idx],
                 ...
                 label=f'Nearest Sample (d={distances[min_dist_idx]:.2f})')

        # 标记出采样点中数值最小的点 (Min Value Sample)
        min_e_idx = energys_np.argmin()
        ax2.plot(distances[min_e_idx], energys_np[min_e_idx],
                 'rx', markersize=12, markeredgewidth=2.5,
                 label=f'Min Value Sample (d={distances[min_e_idx]:.2f})')

        corr = np.corrcoef(distances, energys_np)[0, 1]
        ax2.set_xlabel('Distance to GT')
        ax2.set_ylabel('Value')
        ax2.set_title(f'Value vs Distance\nCorr: {corr:.3f}')
        ax2.legend(fontsize=8)
        ax2.grid(True, alpha=0.3)

        # ===== 子图3: 值 vs 旋转 (Rotation Distribution) =====
        ax3 = axes[2]

        rot_diff = np.abs(coords_np[:, 2] - gt_np[2])
        rot_errors = np.minimum(rot_diff, 2 * np.pi - rot_diff)
        rot_deg = np.rad2deg(rot_errors)
        ...
        ax3.plot(rot_deg[min_rot_idx], energys_np[min_rot_idx],
                 'g*', markersize=15, markeredgecolor='white',
                 label=f'Best Rot Sample')

        # 标记数值最小的采样点在旋转图上的位置
        ax3.plot(rot_deg[min_e_idx], energys_np[min_e_idx],
                 'rx', markersize=12, markeredgewidth=2.5,
                 label=f'Min Value Sample')

        rot_corr = np.corrcoef(rot_errors, energys_np)[0, 1]
        ax3.set_xlabel('Rotation Error (deg)')
        ax3.set_ylabel('Value')
        ax3.set_title(f'Value vs Rotation\nCorr: {rot_corr:.3f}')
    ...
    def visualize_energy_field_local(self, query_feat=None, gt_coord_4d=None, scale_fixed=None,
        ...
        coords_sampled_6d = self.coord_normer.raw_to_net(coords_sampled_4d, append_linear_rot=True)
    ...
    def train(self):
        ...
        self.normed_sigmas = self.coord_normer.get_linear_sigmas( self.gs_sigma_nrc,self.gs_sigma_radrot,self.gs_sigma_logscale)
        ...
        for epoch in range(begin_epoch, num_epochs):
            ...
            for it, batch_uav in tqdm.tqdm(enumerate(self.uav_dataloader_train)):
                ...
                #采样 for de_loss
                n_neighbors=32
                coords_neighbor_linear = self.gs_sampler.sample_importance(coords_gt_linear.to(self.device),num_samples=n_neighbors,include_center=True)
                #采样 for neg_cl_loss
                coords_all_grid, coords_all_grid_labels = self.subspace_sampler.sample_all_subspaces_gpu(n_points_per_subspace=1, use_fine=False)
                n_rand = int(coords_all_grid.shape[0]/8)
                ...
                #compute weight
                coords_ref_linear = torch.concatenate([coords_neighbor_linear,coords_rand_linear.permute(1,0,2).expand(coords_neighbor_linear.shape[0],-1,-1)],dim=1)
                weights_ref = self.coord_normer.compute_weight_matrix_linear(coords_gt_linear.unsqueeze(1),coords_ref_linear,self.normed_sigmas,ignore_dim=[3]).squeeze()
                #debug:
                # coords_all_grid_linear = self.coord_normer.raw_to_linear(coords_all_grid.squeeze())
                # coords_all_grid_linear = torch.concatenate([coords_neighbor_linear[0],coords_all_grid_linear],dim=0)
                # weights2vis = self.coord_normer.compute_weight_matrix_linear(coords_gt_linear[0:1].unsqueeze(1),
                #                                                                      coords_all_grid_linear,
                #                                                                      self.normed_sigmas,
                #                                                                      ignore_dim=[3]).squeeze()
                # coords_all2vis = self.coord_normer.linear_to_raw(coords_all_grid_linear)
                # self.visualize_energy_of_coords(coords_all2vis.squeeze(),weights2vis,coord_gt=coords_gt[0],mode='min',save_path='/home/data/zwk/pyproj_neuloc_v0/trainers/.exps/stage3_neural_proxy_nr40_nc30_r36_31/weight.html')
                ...
                coords_neighbor_net_flat = self.coord_normer.linear_to_net(coords_neighbor_linear_flat)
                feats_vis_neighbor = feats_vis.unsqueeze(1).expand(-1, coords_neighbor_linear.shape[1], -1)
                feats_vis_neighbor = feats_vis_neighbor.reshape(-1, feats_vis.shape[-1])
                dist_neighbor = self.projector(feats_vis_neighbor, coords_neighbor_net_flat)
                dist_neighbor = dist_neighbor.view(bsz, coords_neighbor_linear.shape[1])
                feat_dist_chunks.append(dist_neighbor)

                coords_rand_net_flat = self.coord_normer.linear_to_net(coords_rand_linear.view(-1, 4))
                rand_chunk = 1024
                for start_idx in range(0, coords_rand_net_flat.shape[0], rand_chunk):
                    end_idx = min(start_idx + rand_chunk, coords_rand_net_flat.shape[0])
                    coords_chunk = coords_rand_net_flat[start_idx:end_idx]
                    coords_chunk = coords_chunk.unsqueeze(0).expand(bsz, -1, -1).reshape(-1, 5)
                    feats_vis_chunk = feats_vis.unsqueeze(1).expand(-1, end_idx - start_idx, -1)
                    feats_vis_chunk = feats_vis_chunk.reshape(-1, feats_vis.shape[-1])
                    dist_chunk = self.projector(feats_vis_chunk, coords_chunk)
                    dist_chunk = dist_chunk.view(bsz, end_idx - start_idx)
                    feat_dist_chunks.append(dist_chunk)

                feat_dist = torch.cat(feat_dist_chunks, dim=1)

                # 应用激活函数（可调试控制）
                feat_dist = self.softplus(feat_dist)
                # tau = 0.1  # 这个超参数很关键，0.07~0.1 是常用值，也可以设为可学习参数
                # logits = -feat_dist / tau  # [B, N_samples]
                # 使用 LogSoftmax 计算预测的对数概率
                # import torch.nn.functional as F
                # log_probs_pred = F.log_softmax(logits, dim=1)  # [B, N_samples]

                # 构建目标分布 P 和 构建预测分布 Q
                # target_probs = weights_ref / (weights_ref.sum(dim=1, keepdim=True) + 1e-8)
                # T = getattr(self, 'kl_temperature', 0.025)
                # pred_logits = -feat_dist / T
                ...
                loss_neg = sw_msloss(feat_dist,weights_ref,1-weights_ref) #
                loss_pos = (weights_ref[:,:n_neighbors]*feat_dist[:,:n_neighbors]).mean()
                loss = loss_pos + loss_neg

                # # loss_pos,loss_neg = gc_loss(feat_dist,weights_ref,1-weights_ref)
                # # loss = loss_pos + loss_neg
                # # loss_pos,loss_neg = gc_loss(feat_dist,weights_ref,1-weights_ref)

                # loss = whm_loss(feat_dist,weights_ref,1-weights_ref)
                # loss = dwh_loss(feat_dist,weights_ref,1-weights_ref)
