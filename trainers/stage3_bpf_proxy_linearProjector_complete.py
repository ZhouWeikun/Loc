...
class MetricNetTrainer(GridHashFitTrainer):
    ...
    def analyze_feat_freq_band(self, n_points_per_subspace=1, use_fine=False,vis=False):
        ...
        from util_fft_analyse import analyse_feature_frequency
    ...
    def analyze_energy_field(self, n_nr=128, n_nc=128, use_train_uav=True, local_zoom_wh=None, vis=False,
                                       use_vis_ref=False, chunk_size_vis=1024, analyse_fft=False):
        """
        ...
        if vis:
            ...
            dir2save,epoch = self._get_dir2save(ret_epoch=True)
            suffix = f'hw{local_zoom_wh[0]:.1f}' if local_zoom_wh is not None else 'global'

            from vis_featmap import vis_girddata_in_3d_surface,vis_griddata_in_3d_surface_interactive
            projector_path2save = os.path.join(dir2save,f'ep{epoch}_energy_projector_{set}_{idx}_ns{n_nr}_{suffix}.html')
            ...
            contour_p2save =os.path.join(dir2save,f'contour_{set}_id{idx}_ns{n_nr}_hw{local_zoom_wh[0]:.2f}_visencoder&ingp.png')
            plot_contour(dist_ingp=energy_ingp.detach().cpu(),dist_proj=energy_projector.detach().cpu(),
                                  ...
                                  flow_mode='ascent',unified_scale=False,
                                    save_path=os.path.join(dir2save,f'contour_{set}_id{idx}_ns{n_nr}_hw{local_zoom_wh[0]:.2f}_ingp&projector.png'))
    ...
    def test(self, use_train_uav=False):
        ...
        # self.analyze_energy_field(vis=True)
        ...
        if stage3_ckpt_path:
            ...
            dataset_type = 'train' if use_train_uav else 'test'
            viz_save_path = os.path.join(self.opt.exps_dir,f"energy_field_{dataset_type}.html")
    ...
    def train(self):
        ...
        from losses.CL_loss_fm_weight import SoftMultiSimLoss_WeightedMax,SoftMultiSimLoss_LogSum_HardMining,SoftMultiSimLoss_LogSum,WeightedDirichletEnergyLoss
        self.sms_loss = SoftMultiSimLoss_LogSum_HardMining(init_beta=10.0, margin=0.1, learnable_beta=True).to(self.device)
