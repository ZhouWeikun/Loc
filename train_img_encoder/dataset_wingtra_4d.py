...
#
from util_mk_data_transform import mk_pil_transform,mk_tensor_transform
...
class SatDataset(object):
    ...
    def crop_satimg_by_4d_coords(self, coords_4d, apply_rotation=True):
        """
        Crop satellite images based on given 4D coordinates (nrc, rotation, scale)

        优化版：使用向量化操作和batch_rotate_images_per_sample，消除循环
        ...
        Args:
            ...
            apply_rotation: bool, whether to apply the specified rotation (default: True)
        ...
        if torch.is_tensor(coords_4d):
            coords_4d_np = coords_4d.cpu().numpy()
        else:
            coords_4d_np = coords_4d

        # Handle single coordinate vs batch
        single_input = (coords_4d_np.ndim == 1)
        if single_input:
            coords_4d_np = coords_4d_np[None, :]  # [4] -> [1, 4]

        n_samples = coords_4d_np.shape[0]

        # ========== 批量裁剪（仍需循环，因为每个样本的crop size可能不同） ==========
        # TODO: 如果所有样本的scale相同，可以进一步优化为完全向量化
        satimgs_list = []
        ...
        for i in range(n_samples):
            ...
            # Convert scale_ratio back to satimgsize2crop (in pixels)
            ...
            satimg = self.crop_satimg_by_nrc(nrc, type='tensor', satimgsize2crop=satimgsize2crop)
            # satimg shape: [C, H, W]

            # Resize to network input size
            satimg = self.scale_transform(satimg)  # [C, H, W]

            satimgs_list.append(satimg)

        # Stack all cropped images
        satimgs = torch.stack(satimgs_list, dim=0)  # [N, C, H, W]

        # ========== 批量旋转（消除循环！） ==========
        if apply_rotation:
            # 提取所有旋转角度
            rots_rad = coords_4d_np[:, 2]  # [N]
            rots_deg = np.rad2deg(rots_rad)  # [N]

            # 批量旋转：每个图像旋转不同角度
            satimgs = batch_rotate_images_per_sample(satimgs, rots_deg)  # [N, C, H, W]

        # If single input, remove batch dimension
        if single_input:
            satimgs = satimgs[0]  # [C, H, W]
...
class UAVDataset(object):
    def __init__(self,
        ...
        self.uav_scales = np.array(h_cover_m[scale_mask] / scale_ref_m)
        ...
        self.uav_nrcs_torch = torch.from_numpy(self.uav_nrcs).to(torch.float32)
        self.uav_rots_torch = torch.from_numpy(self.uav_rots).to(torch.float32)[...,None]
        self.uav_scales_torch = torch.from_numpy(self.uav_scales).to(torch.float32)[...,None]
        ...
        self.uav_transform_train = mk_pil_transform_with_params(
            ...
            imgsize2net=self.imgsize2net,
            rand_rot=True,
            rand_scale=True, scale_range=(0.95, 1.05),
            rand_affine=True, affine_para={'degrees': 2.5, 'translate': (0.05, 0.05), 'scale': (0.95, 1.05), 'shear': 10},
            rand_erase=True, center_crop=True, rand_crop=False)
    ...
    def split_uav_dataset(self, train_radio=0.9):
        ...
        #     self.uav_nrcs_torch_train = self.uav_nrcs_torch[:n_train]
        #     self.uav_nrcs_torch_test = self.uav_nrcs_torch[n_train:]
        #
        # # === Split rotation（新增）===
        # if hasattr(self, 'uav_rots'):
        #     self.uav_rots_train = self.uav_rots[:n_train]
        #     self.uav_rots_test = self.uav_rots[n_train:]
        #     self.uav_rots_torch_train = self.uav_rots_torch[:n_train]
        #     self.uav_rots_torch_test = self.uav_rots_torch[n_train:]
        #
        # # === Split scale（新增）===
        # if hasattr(self, 'uav_scales'):
        #     self.uav_scales_train = self.uav_scales[:n_train]
        #     self.uav_scales_test = self.uav_scales[n_train:]
        #     self.uav_scales_torch_train = self.uav_scales_torch[:n_train]
        #     self.uav_scales_torch_test = self.uav_scales_torch[n_train:]
