...
class Trainer(object):
    ...
    def _get_feat_gallery_fm_grid(self,overlap=0.5,delta_rot_rangle=10,scale=None):
        with torch.no_grad():
            ...
            dict2ret = {
                'feat_gallery':feat_gallery,
                'feat_gallery_faltter':feat_gallery_flatten,
    ...
    def _opt_grid_coord(self,coords2opt,encoded_coords_top1,feat_q):
        condition_features = encoded_coords_top1.to(feat_q.device)
        coords_4d = torch.tensor(coords2opt, dtype=torch.float32, requires_grad=True).to(feat_q.device)
        optimizer = torch.optim.Adam([coords_4d], lr=1e-4)
        num_optimization_steps = 100
        for i in range(num_optimization_steps):
            # 1. 清空之前的梯度
            optimizer.zero_grad()
            feat_ref = self._feats_fm_grid(coords_4d)
            feat_ref = self.grid_mlp(inputs=feat_ref, condition_features=condition_features)
            ...
            optimizer.step()
            if i % 10 == 0:
                print(f"Step {i}, Loss: {loss.item()}, Coords: {coords_4d.data}")
    ...
    def test_xy_rot(self):
        ...
        if os.path.exists(p2gallery_mat):
            ...
            feat_gallery_flatten = torch.tensor(gallery_mat['feat_gallery'],dtype=torch.float32).flatten(start_dim=0, end_dim=2)
            ...
            nrcs_gallery = self.sat_dataset.crop_sat_unifrom(size2clip=self.sat_dataset.satimgsize2crop_mean, overlap=overlap, only_nrcs=True)
            nrcs_gallery_flatten = torch.tensor(nrcs_gallery, dtype=torch.float32).flatten(start_dim=0, end_dim=1)
            delta_rot_rangle = 10
            rot_angles = [-180 + delta_rot_rangle * i for i in range(360 // delta_rot_rangle)]
            rots_ref = torch.tensor(np.deg2rad(np.stack(rot_angles)), dtype=torch.float32)
            ...
            coords_top1 = coords_gallery_flatten[pred_id[:,0]]
            # coords_top1_opted = self._opt_grid_coord(coords_top1,feat_q)

            feat_q = feat_q.to(self.device)
            encoded_coords_top1 = coords_gallery_encoded_flatten[pred_id_rc[:,0]]
            condition_features = encoded_coords_top1.to(feat_q.device)
            coords2opt = torch.tensor(coords_top1,dtype=torch.float32,device=feat_q.device, requires_grad=True)  # 在这里设置需要梯度
            ...
            nrc_loc_success = create_success_mask(pred_id_rc[:,:1], uav_labels_query, k=1)
            err_dist_top1 = torch.norm(coords_top1[nrc_loc_success]-coords_q[nrc_loc_success],dim=-1)
            err_dist_top1_opted = torch.norm(coords2opt.detach().cpu()[nrc_loc_success]-coords_q[nrc_loc_success],dim=-1)
            print(f'err_dist_top1:{err_dist_top1.mean().item():.5f};err_dist_top1_opted:{err_dist_top1_opted.mean().item():.5f}')
            ...
            rot_recall_results = compute_recall_by_label(
                q_labels=gt_ids_rot[nrc_loc_success].unsqueeze(-1),
                pred_labels_per_query=pred_ids_unraled[2][nrc_loc_success,:10],
                k_values=[1, 3, 5, 10],
                title="Conditional Rotation Recall (based on radius search)"
            )
            print("\n条件旋转召回率结果:", rot_recall_results)

            gtrot2eval = rots_q[nrc_loc_success]
            predrot2eval= rots_ref[pred_ids_unraled[2][nrc_loc_success,0]]
