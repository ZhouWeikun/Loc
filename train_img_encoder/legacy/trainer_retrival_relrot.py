...
from tool.utils_fm_duav import copyfiles2checkpoints, get_logger
...
def get_parse():
    ...
    #about exp setting
    parser.add_argument('--exp_name', default='debug_zuchwil',type=str, help='the experiment name that will be saved in exps dir in the root')
    ...
    parser.add_argument('--p_satinfo_json',
                        default='/home/data/zwk/data_uavimgs_wingtra/Zuchwil/SWISSIMAGE2024_30blocks_res03m.json',
                        type=str, help='training dir path')
    parser.add_argument('--p_uavinfo_json',
                        default='/home/data/zwk/data_uavimgs_wingtra/Zuchwil/uavimgs_info.json',
                        type=str, help='training dir path')
    ...
    group_info = {
        'exp_setting': ['p_yaml', 'p_satinfo_json', 'p_uavinfo_json','exp_name', 'load2train', 'load2test', 'val','val_freq', 'save_freq', 'tensorboard'],
...
class Trainer(object):
    ...
    def train(self):
        ...
        # backup files
        # if not (len(opt.load2train)>0 and os.path.dirname(opt.load2train).split('/')[-1]==opt.exp_name):
        copyfiles2checkpoints( self.opt )

        # config logger
        logger = get_logger("exps/{}/train.log".format(opt.exp_name),'trainer_logger')
        self.logger = logger
        # config tensorborad
        ...
        for epoch in range(begin_epoch,num_epochs):
            ...
            for it,data in tqdm.tqdm(enumerate(dataloader)):
                ...
                rc_radius = self.dataloader.dataset.halfimg_radius_rc * 0.5
    ...
    def val(self,overlap=0.75):
        ...
        if self.opt.dataset_name == 'gta':
            ...
            with torch.no_grad():
                sat_tiles, rc_gallery = self.dataloader.dataset.clip_sat_unifrom(overlap=overlap)
                sat_tiles = sat_tiles.reshape(-1,* sat_tiles.shape[2:]).to(self.device)
                split_size = 32
                feat_gallery = []

                for tiles in tqdm.tqdm(torch.split(sat_tiles, split_size, dim=0)):
                    output = self.model(tiles,ret_sg=False)
                ...
                feat_query,rc_query = [],[]
                self.dataloader.dataset.switch_stage('test')
                self.dataloader.collate_fn = torch.utils.data._utils.collate.default_collate
                dataloader = self.dataloader
                ...
                from datasets_custom.make_dataloader_xmu import qurey_label_fm_gallery_rc
                distrc_sats2uav,uav_labels_query = qurey_label_fm_gallery_rc(rc_gallery.reshape(-1,2), rc_query, self.dataloader.dataset.halfimg_radius_rc)
                ...
                self.dataloader.dataset.switch_stage('train')
                from datasets_custom.make_dataloader_xmu import train_collate_fn
                self.dataloader.collate_fn = train_collate_fn
    ...
    def _test_ready(self):
        ...
        config_path = os.path.dirname(opt.load2test) + os.sep + 'opts.yaml'
        ...
        self.model.eval()
        self.dataloader = make_dataloader_xmu(opt, stage='test') if not hasattr(self, 'dataloader') else self.dataloader
        return self.model,self.dataloader


    def output_test_res(self,overlap=0.75,with_sg=False):
        model, dataloader = self._test_ready()

        # get feat_gallery from satimgs
        ...
        for tiles in tqdm.tqdm(torch.split(sat_tiles, split_size, dim=0)):
            if with_sg:
                output,output_sgs =  model(tiles,ret_sg=True)
                sg_gallery.append(output_sgs)
            else:
                output = model(tiles, ret_sg=False)
            output = output[1] if type(output)==list else output
            feat_g =  output.detach().cpu()
            feat_g =  feat_g.view(feat_g.shape[0],-1) if len(feat_g.shape)>2 else feat_g
            feat_gallery.append(feat_g)
        feat_gallery = torch.cat(feat_gallery, dim=0) #.reshape(*rcs_girdcoord_center.shape[:2],-1)  # 形状 [N1*N2, D]
        latlon_gallary = dataloader.dataset.nrc_to_latlon(rc_gallery)
        ...
        feat_query = torch.cat(feat_query,dim=0)
        rc_query = np.concatenate(rc_query,axis=0)
        latlon_query = dataloader.dataset.nrc_to_latlon(rc_query)
        ...
        result = {'gallery_feat': feat_gallery,
                  'gallery_rc': rc_gallery,
                  'gallery_latlon': latlon_gallary,
                  'gallery_hw':np.array(rc_gallery.shape[:2]),
                  'query_feat': feat_query,
                  'query_rc': rc_query,
                  ...
                  'radius_meter': dataloader.dataset.halfimg_radius_meter,
                  }
        if with_sg:
        ...
        suffix = f"_overlap{overlap}_radius{dataloader.dataset.halfimg_radius_meter:.0f}m.mat"
        # scipy.io.savemat(self.opt.checkpoint.replace('.pth', suffix), result) # todo: change to exp dir
        scipy.io.savemat( f"exps/{self.opt.exp_name}/{os.path.basename(self.opt.load2test).replace('.pth', suffix)}" ,result)
