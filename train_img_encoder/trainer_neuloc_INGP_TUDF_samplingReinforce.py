...
class Trainer(object):
    ...
    def train(self):
        ...
        for epoch in range(begin_epoch,num_epochs):
            ...
            for it,data in tqdm.tqdm(enumerate(self.sat_dataloader)):
                ...
                coords_mixed_flatten = coords_mixed.reshape(-1,coords_mixed.shape[-1])

                # 3.结果预测
                # get feats from hash
                feats_grid = self._feats_fm_grid(coords_mixed)
                coords_mixed_encoded = encode_4d_coords(coords_mixed,
                                                  rc_encoder=self.rc_pos_encoder,
                                                  rot_endcoder=self.rot_pos_encoder,
                                                  scale_encoder=self.scale_pos_encoder)
                coords_mixed_encoded_flatten = coords_mixed_encoded.reshape(-1,coords_mixed_encoded.shape[-1])
                feats_grid_flatten = feats_grid.reshape(-1,feats_grid.shape[-1])
                feats_grid_flatten = self.grid_mlp(inputs=feats_grid_flatten,
                                           condition_features= coords_mixed_encoded_flatten.to(feats_grid_flatten.device))
                                           # condition_features=torch.concatenate([feats_grid, coords_encoded.to(feats_grid.device)],dim=-1))
                # feats_grid_flatten = torch.nn.functional.normalize(feats_grid_flatten,dim=-1)


                # compute dist_mat form coords
                rc_dist_mat = gt_coords[:,:2].unsqueeze(1) - coords_mixed_flatten[:,:2].unsqueeze(0)
                rc_dist_mat = torch.norm(rc_dist_mat,dim=-1)
                r_dist_mat = gt_coords[:,2].unsqueeze(1) - coords_mixed_flatten[:,2].unsqueeze(0)
                r_dist_mat = torch.abs(torch.atan2(torch.sin(r_dist_mat), torch.cos(r_dist_mat))).squeeze() #atan2 函数的输出范围是 [-π, π]
                s_dist_mat = gt_coords[:,3].unsqueeze(1) / coords_mixed_flatten[:,3].unsqueeze(0)
                ...
                for query_chunk in torch.split(feats_q, chunk_size, dim=0):
                    ...
                    ref_expanded = feats_grid_flatten.unsqueeze(0).expand(chunk_size_actual, feats_grid_flatten.shape[0], -1)  # [chunk, B, D]
                    coords_encoded_expanded = coords_mixed_encoded_flatten.unsqueeze(0).expand(chunk_size_actual, coords_mixed_flatten.shape[0], -1).to(device)  # [chunk, B, 4]
                    # 计算这一块的距离
                ...
                coords_encoded_expanded = coords_mixed_encoded_flatten.unsqueeze(0).expand(B, B, -1).to(ref_expanded.device)  # [B, B, coord_dim]
    ...
    def get_feats_fm_imgs(self, satimgs_flatten, feat_fm_agg=True):
        batchsize = 512
        feat_galllery = []
    ...
    def test_xy_rot_fm_metric_net(self):
        ...
        with torch.no_grad():
            ...
            satimgs_q = self.sat_dataset.crop_satimgs_by_4d_coords(coords_q)
            feat_q = self.get_feats_fm_imgs(satimgs_q, feat_fm_agg=True)
