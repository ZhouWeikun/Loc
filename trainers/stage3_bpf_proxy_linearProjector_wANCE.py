...
class MetricNetTrainer(GridHashFitTrainer):
    ...
    def _build_cma_init_seeds(
        ...
        """
        Build CMA-ES initial seeds from coarse 4D scores.

        Strategy:
        1. Marginalize scale to get 3D anchor scores.
        2. Select Top-N 3D anchors.
        3. Resolve the scale inside each anchor by:
           - `argmax`: choose the best discrete scale bin
           - `log_expectation`: compute weighted mean in log-scale space
        """
        n_nr, n_nc, n_rot, n_scale = [int(x) for x in n_coarse]
        topk_seed = min(int(topk_seed), n_nr * n_nc * n_rot)
        if topk_seed <= 0:
    ...
    def _test_3d_fine_accuracy_coarse2fine(
        ...
        coords_topN_l0_lefted = coords_candidates_flat[flat_indices_4d]
    ...
    def _test_3d_fine_accuracy_CMA_ES(
        ...
        """
        V2: L0 粗筛 + 多起点 CMA-ES 精修 的 3D 细粒度定位评估。
        ...
        参数说明（调参建议）:
            ...
            cma_variant (str):
                CMA 后端方案: {'CMA','LRA-CMA'}。
                - 'CMA': 标准 CMA-ES（当前默认）
                - 'LRA-CMA': 启用 cmaes 的学习率自适应 (lr_adapt=True)
            cma_enable_early_stop (bool):
                是否启用 CMA 后端 `should_stop()` 早停逻辑。
                开启后可能减少实际迭代代数，提升速度。
            ...
            cma_elite_ratio (float):
                兼容参数位（便于后续切换后端时保持接口稳定）。
                当前 `cmaes` 标准 CMA 后端不直接暴露该参数。
        ...
        from trainers.util_stage3_multi_start_CMAES_refine import MultiStartCMAESRefiner
        cma_refiner = MultiStartCMAESRefiner(coords_processor=self.coord_normer, device=self.device)

        def _score_fn_for_refiner(query_feat_1xc, coords_raw_nx4):
            with torch.no_grad():
                scores = evaluator.score_candidates(
                    query_feats=query_feat_1xc,
                    coords_batch=coords_raw_nx4,
                    mode=cma_prob_mode,
                    normalize=False,
                )
        ...
        if cma_variant_norm in ("cma", "standard", "base"):
            cma_variant = "CMA"
        elif cma_variant_norm in ("lra-cma", "lracma", "lra_cma", "lra"):
            cma_variant = "LRA-CMA"
        else:
            raise ValueError(f"cma_variant must be 'CMA' or 'LRA-CMA', got {cma_variant_raw}")
        ...
        print(f"\n{'=' * 60}")
        print("3D细粒度定位测试 (CMA-ES Multi-Start)")
        ...
        for q_idx in range(num_queries):
            ...
            if refine_loc and cma_backend_available:
                try:
                    coords_refined, score_refined = cma_refiner.refine_single_query(
                        query_feat=feat_q,
                        seed_coords_raw=seed_coords,
                        score_fn=_score_fn_for_refiner,
                        sigma0=cma_sigma0,
                        popsize=cma_popsize,
                        n_iterations=cma_iters,
                        elite_ratio=cma_elite_ratio,
                        maximize=True,
                        cma_seed=q_idx,
                    ...
                    print(f"[V2] CMA backend unavailable ({err}). Fallback to seed ranking.")
                    coords_refined = seed_coords
                    with torch.no_grad():
                        score_refined = evaluator.score_candidates(
                            query_feats=feat_q,
                            coords_batch=coords_refined,
                            mode=cma_prob_mode,
                            normalize=False,
                        ).squeeze(0)
                        score_refined = torch.nan_to_num(score_refined, nan=-1e9, posinf=1e9, neginf=-1e9)
            else:
                coords_refined = seed_coords
                with torch.no_grad():
                    score_refined = evaluator.score_candidates(
            ...
            rank_idx = torch.argsort(score_refined, descending=True)
            coords_ranked = coords_refined.index_select(0, rank_idx)[:topk_seed]
            scores_ranked = score_refined.index_select(0, rank_idx)[:topk_seed]

            if coords_ranked.shape[0] < topk_seed:
                pad_n = topk_seed - coords_ranked.shape[0]
                pad_coords = seed_coords[:pad_n]
                pad_scores = seed_scores[:pad_n]
                coords_ranked = torch.cat([coords_ranked, pad_coords], dim=0)
                scores_ranked = torch.cat([scores_ranked, pad_scores], dim=0)

            final_coords_list.append(coords_ranked)
            final_scores_list.append(scores_ranked)
        ...
        coords_pred_all = torch.stack(final_coords_list, dim=0)
        probs_pred_all = torch.stack(final_scores_list, dim=0)
    ...
    def analyze_energy_field(self, n_nr=128, n_nc=128, use_train_uav=True, local_zoom_wh=None, vis=False,
        ...
        if vis:
            ...
            plot_contour(dist_ingp=energy_ingp.detach().cpu(),dist_proj=energy_projector.detach().cpu(),
                                  gt_coords=(gt_nr_idx, gt_nc_idx),crop_size=0,with_flow=False,
    ...
    def test(self, use_train_uav=False):
        ...
        # self.analyze_energy_field(n_nr=int(40/2),n_nc=int(30/2),vis=True,query_id=10)
        ...
        level_resample_cfgs = [
            {"resample_dims": (4, 4, 2, 1), "topN": 128, "space_scale": 1.0},
            {"resample_dims": (5, 5, 2, 1), "topN": 128, "space_scale": 1.0},
            {"resample_dims": (4, 4, 2, 4), "topN": 128, "space_scale": 1.0},
        ]
        ...
        #analyze:
        probs = dict_res['probs_fm_dist']
        coords_pred = dict_res['coords_pred']
        coords_gt = dict_res['coords_gt']
        dist_2d=torch.norm(coords_pred[:,0,:2]-coords_gt[:,:2],dim=-1)
        dist_2d_mask = dist_2d<self.sat_datasets['zurich'].halfimg_radius_nrc
    ...
    def train(self):
        ...
        loss_type = str(getattr(opt, "loss_type", "sms")).lower()
        use_sws_loss = loss_type in {"sms", "sws_loss"}

        from losses.CL_losses_w_weight import pairLoss_singleEdge_weightedHardest,SoftHardTripleLoss,WeightedDirichletEnergyLoss
        # 其他候选loss：
        self.wde_loss = WeightedDirichletEnergyLoss(apply_log=True)
        if use_sws_loss:
            self.sms_loss = SoftHardTripleLoss(decoupling=False).to(self.device)
        ...
        # about mining
        self.use_hard_neg_mining = getattr(opt,'use_hard_neg_mining',True)
        self.hard_neg_topN = getattr(opt,'hard_neg_topN',1024)
        self.query_uav_only = getattr(opt,'query_uav_only',True)
        ...
        for epoch in range(begin_epoch, num_epochs):
            ...
            for it, batch_uav in tqdm.tqdm(enumerate(self.uav_dataloader_train)):
                ...
                if use_sws_loss:
                    coords_gt_net = self.coord_normer.linear_to_net(coords_gt_linear)
                    coords_gt_6d = torch.cat(
                        [coords_gt_net, coords_gt_linear[:, 2:3]],
                        dim=-1
                    )
                    grid_input_gt = torch.cat(
                        [coords_gt_6d[:, :2], coords_gt_6d[:, -1:]],
                        dim=-1
                    )
                    with torch.no_grad():
                        feats_grid_raw = self._get_feats_fm_grid(grid_input_gt)
                        coords_encoded = self.pos_encoder_grid(coords_gt_6d[:, :5])
                        feats_grid_gt = self.grid_mlp(
                            inputs=feats_grid_raw,
                            condition_features=coords_encoded
                        )
                        feats_grid_gt = TF.normalize(feats_grid_gt, dim=-1)
                    dist_pos = self.projector.compute_energy(
                        feats_vis, feats_grid_gt, metric='euclidean'
                    )
                    if dist_pos.dim() == 1:
                        dist_pos = dist_pos.unsqueeze(1)
                    feat_dist_chunks.append(dist_pos)
                    pos_cols = dist_pos.shape[1]
                else:
                    ...
                    pos_cols = coords_neighbor_linear.shape[1]
                ...
                if use_sws_loss:
                    total_cols = feat_dist.shape[1]
                    if (not hasattr(self, "pos_mask_mat")
                            or self.pos_mask_mat.shape[0] != bsz
                            or self.pos_mask_mat.shape[1] != total_cols):
                        self.pos_mask_mat = torch.zeros(
                            (bsz, total_cols),
                            device=feat_dist.device,
                            dtype=torch.bool
                        )
                        self.pos_mask_mat[:, :pos_cols] = True
                    loss = sms_loss(feat_dist, self.pos_mask_mat)
                else:
